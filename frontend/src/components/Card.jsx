...
export default function Card({ url = "" }) 
{
    ...
    return (
        <div className="bg-white p-4 rounded shadow max-w-xs hover:shadow-lg transition">
            {/* Imagen del pokémon */}
            <img src={pokemon.sprites.other["official-artwork"].front_default} className="h-40 mx-auto" alt={pokemon.name}/>
            ...
            <div className="flex justify-center gap-2 mt-2">
                ...
                <span key={type.type.name} className="px-2 py-1 text-white text-xs rounded" style={{ backgroundColor: `var(--type-${type.type.name})` }}>
                    {type.type.name}
                </span>
                ))}
            </div>
